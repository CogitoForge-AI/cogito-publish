...
Cogito Publish được gọi như một lệnh: `cogito-publish [--site PATH] <command> [options]`.
Điểm vào theo module Python vẫn là `python -m pyssg ...`.

## Tùy chọn toàn cục

| Tùy chọn | Mặc định | Mô tả |
|---|---|---|
| `--site PATH` | `.` | Thư mục trang. Mọi đường dẫn khác trong cấu hình (`content_dir`, `output_dir`, `layout`) đều tương đối so với nó. Đây là tùy chọn toàn cục nên đặt *trước* lệnh: `cogito-publish --site my-site build`. |
...
`cogito-publish deploy <target-or-action>`, với phần cuối là một target đã cấu hình
