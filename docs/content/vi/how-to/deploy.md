...
## 3. Triển khai bằng `cogito-publish deploy`

Cogito Publish có sẵn lệnh deploy tích hợp cho các host phổ biến. Khai báo tùy chọn theo
từng target dưới `Config.deploy` (khóa là tên target), rồi chạy
`cogito-publish deploy <target>`. Lệnh sẽ build trang, tải đầu ra lên, và ghi lại kết quả
...
```bash
cogito-publish --site my-site deploy list           # các target đã cấu hình + cái nào đã hiện thực
cogito-publish --site my-site deploy github-pages   # build và xuất bản
cogito-publish --site my-site deploy status         # bản ghi lần deploy gần nhất theo target
```
...
- **Netlify / Cloudflare Pages / Vercel** - đặt lệnh build là
  `cogito-publish --site my-site build` và thư mục xuất bản là
  `my-site/dist`.
...
Build của Cogito Publish là tất định: với cùng đầu vào, hai lần build cho ra kết quả giống
