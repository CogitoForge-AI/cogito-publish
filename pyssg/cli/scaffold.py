"""Site scaffolding for ``cogito-publish new``.
...
from nothing to ``cogito-publish build`` in one step. ``eject_layout`` copies a built-in
...
2. Run `cogito-publish build` to render the site into `dist/`.
3. Run `cogito-publish serve` to preview with live reload.
"""
...
- Run `cogito-publish build` to render, or `cogito-publish serve` for live preview.
"""
...
def init_site(site_dir: Path, *, preset: str, force: bool = False) -> list[Path]:
    ...
    so re-running ``new site`` in a real site does not clobber it.
    """
...
Generated by ``cogito-publish new plugin``. A plugin is a class plus a thin lowercase
